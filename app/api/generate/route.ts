...
• **${L.cognitive}:** [Bloom's verb] + specific skill
• **${L.psychomotor}:** [Bloom's verb] + specific skill  
• **${L.affective}:** value or attitude to develop
...
**${L.warmup}:** Write the full question using a specific ${city} place, person, or event. Then write the expected complete student answer.`;
    ...
    const promptC = `Write SECTION C of an ILAW lesson plan. Output ONLY these 3 sections in order, fully written. Never skip any session or any Part.
Each section key must be on its OWN line alone — never inline with content. Do not append a colon to the main section key lines.

${lessonHeader}

FLOW
Write Parts 1, 2, and 3 for EVERY session listed in SESSIONS above. Use this exact structure:

**${L.session} N — "Session Title" (total duration)**

**${L.part} 1 — "Activity Name" (time)**
**${L.objLink}:** State the exact learning objective this Part addresses.
**${L.teacherScript}:**
• [Opening line — greet and state today's goal]
• [Connect to prior knowledge with a question]
• [Model the concept using a ${city} example — include real names, amounts, places]
• [Write steps on board: Step 1... Step 2... Step 3...]
• [Mid-activity check question to gauge understanding]
• [Transition line to next activity]
**${L.studentActions}:**
• [What students write/calculate/say at each stage]
• [Expected outputs — be specific: "Students write the formula in their notebooks"]
**${L.examples}:**
...
• **${L.forSupport}:** [Specific scaffold explicitly tied to this mathematical task, e.g., structural breakdown or graphic template]
• **${L.forAdvanced}:** [Genuine higher-order challenge task using complex variables or localized optimization]
**${L.guiding}:**
...
**${L.part} 2 — "Activity Name" (time)**
[Same full structure as Part 1 above]

**${L.part} 3 — ${L.synthesis} (time)**
• **${L.closing}:** Write 3 full discussion questions with expected student responses
• **${L.exit}:** Write the exact exit ticket question + scoring guide (what counts as complete vs incomplete)
...
**${L.tech}:** • Detail 2 accessible digital tool solutions or online platforms (e.g., GeoGebra, Desmos, or specialized educational web apps) complete with absolute URLs that can enhance learning outside of regular classroom hours.`;
    ...
    const promptD = `Write SECTION D of an ILAW lesson plan. Output ONLY these 3 sections in order, fully written. Never skip any session.
Each section key must be on its OWN line alone — never inline with content. Do not append a colon to the main section key lines.
...
After Session 2:
After Session 3:
Notes to share with co-teachers, parents, or school leaders:
I would like my instructional coach to help me with:`;
    ...
    async function callAI(userPrompt: string, callLabel: string): Promise<string> {
      ...
      // OpenRouter, Mistral, Cerebras fallbacks
    ...
    console.log('Starting 4 parallel calls: A(Header) B(PreLesson) C(Flow+Resources+Integration) D(Assessment)');
    ...
    const ALL_TAGS = [
      ...
      'OPPORTUNITIES_FOR_INTEGRATION', 'FORMATIVE_ASSESSMENT', 'WAYS_FORWARD', 'REFLECTIONS',
    ];
